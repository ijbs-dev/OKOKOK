...
router.post("/cliente/new", (req, res) => {
    try {
        ...
        res.status(201).json({ mensagem: "Cliente cadastrado com sucesso!", clientes });
    } catch (error) {
        console.log(error);
...
router.put('/cliente/:id', (req, res) => {
    try {
        ...
        if (index === -1) {
            ...
            //res.status(204).send();
            res.status(201).json({ mensagem: "Cliente atualizado com sucesso!", clientes });
        }
    } catch (error) {
        console.log(error);
...
router.delete('/cliente/:id', (req, res) => {
    ...
    if (index === -1) {
        ...
        res.status(201).json({ mensagem: "Cliente deletado com sucesso!", clientes });
    }
});
